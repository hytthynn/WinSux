...
# disable open file security warning (zone identifier)
cmd /c "reg add `"HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Attachments`" /v `"SaveZoneInformation`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
cmd /c "reg add `"HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Associations`" /v `"LowRiskFileTypes`" /t REG_SZ /d `".avi;.bat;.cmd;.exe;.htm;.html;.lnk;.msi;.mp3;.mp4;.mkv;.rar;.reg;.vbs;.zip;.7z`" /f >nul 2>&1"

# hide hidden files in explorer (desktop.ini etc won't show on desktop)
cmd /c "reg add `"HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced`" /v `"Hidden`" /t REG_DWORD /d `"2`" /f >nul 2>&1"
cmd /c "reg add `"HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced`" /v `"ShowSuperHidden`" /t REG_DWORD /d `"0`" /f >nul 2>&1"
...
# #101010 signout & lockscreen
		## ms-settings:lockscreen
# create #101010 color image
Add-Type -AssemblyName System.Windows.Forms
$screenWidth = [System.Windows.Forms.SystemInformation]::PrimaryMonitorSize.Width
$screenHeight = [System.Windows.Forms.SystemInformation]::PrimaryMonitorSize.Height
Add-Type -AssemblyName System.Drawing
$file = "C:\Windows\Wallpaper101010.jpg"
$edit = New-Object System.Drawing.Bitmap $screenWidth, $screenHeight
$color = New-Object System.Drawing.SolidBrush ([System.Drawing.Color]::FromArgb(16, 16, 16))
...
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\PersonalizationCSP`" /v `"LockScreenImagePath`" /t REG_SZ /d `"C:\Windows\Wallpaper101010.jpg`" /f >nul 2>&1"
...
# #101010 wallpaper
cmd /c "reg add `"HKCU\Control Panel\Desktop`" /v `"Wallpaper`" /t REG_SZ /d `"C:\Windows\Wallpaper101010.jpg`" /f >nul 2>&1"
rundll32.exe user32.dll, UpdatePerUserSystemParameters

# remove home from windows 11 file explorer navigation pane
        ## HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\{f874310e-b6b7-47dc-bc84-b9e6b38f5903}
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer`" /v `"HubMode`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
cmd /c "reg delete `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\{f874310e-b6b7-47dc-bc84-b9e6b38f5903}`" /f >nul 2>&1"
cmd /c "reg delete `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace_36354489\{f874310e-b6b7-47dc-bc84-b9e6b38f5903}`" /f >nul 2>&1"
...
# no extra start menu shortcuts created
...
# keep openssh client and server
...
# enable directplay (legacy component required by older games)
        ## c:\windows\system32\optionalfeatures.exe
Enable-WindowsOptionalFeature -Online -FeatureName "LegacyComponents" -NoRestart -WarningAction SilentlyContinue | Out-Null
Enable-WindowsOptionalFeature -Online -FeatureName "DirectPlay" -NoRestart -WarningAction SilentlyContinue | Out-Null
...
# display
# turn off display after never (0 = never)
powercfg /setacvalueindex 99999999-9999-9999-9999-999999999999 7516b95f-f776-4464-8c53-06167f40cc99 3c0bc021-c8a8-4e07-a973-6b14cbcb2b7e 0x00000000 2>$null
powercfg /setdcvalueindex 99999999-9999-9999-9999-999999999999 7516b95f-f776-4464-8c53-06167f40cc99 3c0bc021-c8a8-4e07-a973-6b14cbcb2b7e 0x00000000 2>$null
...
# rename drive C to SYSTEM
cmd /c "label C: SYSTEM >nul 2>&1"
...
Remove-Item "$env:SystemDrive\DumpStack.log" -Force -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Host "HOTSPOT & SYSTEM PROTECTION`n"
        ## ms-settings:network-mobilehotspot
        ## c:\windows\system32\control.exe sysdm.cpl ,4

# delete mobile hotspot (WiFi Direct virtual adapter)
cmd /c "reg add `"HKLM\SYSTEM\CurrentControlSet\Services\WlanSvc`" /v `"Start`" /t REG_DWORD /d `"4`" /f >nul 2>&1"
Get-NetAdapter | Where-Object { $_.Name -like "*Local Area Connection*" -or $_.Name -like "*Microsoft Wi-Fi Direct*" } |
    ForEach-Object { Disable-NetAdapter -Name $_.Name -Confirm:$false -ErrorAction SilentlyContinue }
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\WcmSvc\Tethering`" /v `"WiFiSharingEnabled`" /t REG_DWORD /d `"0`" /f >nul 2>&1"

# disable system protection (restore points) on all drives
try {
Disable-ComputerRestore -Drive "C:\" -ErrorAction SilentlyContinue | Out-Null
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore`" /v `"RPSessionInterval`" /t REG_DWORD /d `"0`" /f >nul 2>&1"
cmd /c "reg add `"HKLM\SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore`" /v `"DisableSR`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
cmd /c "reg add `"HKLM\SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore`" /v `"DisableConfig`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
# delete existing shadow copies / restore points
cmd /c "vssadmin delete shadows /all /quiet >nul 2>&1"
} catch { }
